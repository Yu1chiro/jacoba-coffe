  ...
  async function showCheckoutForm(productData, userId) {
    ...
    const result = await Swal.fire({
      ...
      html: `
       <div class="space-y-6">
        ...
        <div class="bg-gray-100 p-5 rounded-lg shadow-md">
          <h3 class="text-lg font-bold mb-3">Produk yang Dipesan:</h3>
          <div class="flex gap-4 items-center">
            <img src="${productData.thumbnail}" alt="${productData.name}" class="w-20 h-20 object-cover rounded-lg shadow">
            <div class="flex-1">
              <p class="font-semibold">${productData.name}</p>
              <p class="text-sm text-gray-600">Variant: ${productData.variant}</p>
            </div>
            <div class="text-right">
              <p class="text-lg font-bold">Rp ${new Intl.NumberFormat('id-ID').format(productData.price)}</p>
              <div class="flex items-center gap-2 mt-2">
                <button class="px-3 py-2 bg-gray-300 rounded-lg hover:bg-gray-400 transition" onclick="updateQuantity(-1)">-</button>
                <input type="number" id="quantity" value="${currentQuantity}" min="1" class="w-16 text-center border rounded-lg">
                <button class="px-3 py-2 bg-gray-300 rounded-lg hover:bg-gray-400 transition" onclick="updateQuantity(1)">+</button>
              </div>
              <p class="mt-2 font-medium">Total: Rp <span id="total-price">${new Intl.NumberFormat('id-ID').format(productData.price * currentQuantity)}</span></p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input id="customerName" class="swal2-input rounded-lg border-gray-300 shadow-sm focus:ring focus:ring-gray-300" placeholder="Masukkan nama Anda">
          </div>
          
          <div>
            <label class="block text-sm font-medium mb-1">Nomor Meja:</label>
            <input id="tableNumber" class="swal2-input rounded-lg border-gray-300 shadow-sm focus:ring focus:ring-gray-300" placeholder="Masukkan nomor meja">
          </div>
        </div>
      </div>
      `,
      didOpen: () => {
        ...
        // Fungsi untuk update quantity di UI
    ...
    if (result.isConfirmed) {
      try {
        ...
        // Gunakan fungsi addToCart yang asli
